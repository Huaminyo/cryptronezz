    ...
    <div className="grid gap-4 md:grid-cols-2">
      <section className="glass rounded-xl p-5">
        ...
        <div className="mt-3 grid gap-2">
          ...
          <input className="rounded-lg bg-black/20 p-2" placeholder="Points" type="number" value={form.points} onChange={(e) => setForm((v) => ({ ...v, points: Number(e.target.value) }))} />
          ...
          <input className="rounded-lg bg-black/20 p-2" placeholder="https://..." value={form.link} onChange={(e) => setForm((v) => ({ ...v, link: e.target.value }))} />
          <button onClick={createTask} className="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold">Create Task</button>
        </div>
      </section>

      <section className="glass rounded-xl p-5">
        <h2 className="font-semibold">Disable / Enable Tasks</h2>
